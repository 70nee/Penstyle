...
import {
  ...
  FolderOpen, GripVertical, Hash, ImagePlus, Library, LogOut, Menu, Mic, MicOff, MoveDown, MoveUp,
...
function LoginScreen({ configured, error, onProvider }: { configured: boolean; error: string; onProvider: (provider: Provider) => void }) {
  ...
  return <main className="login-screen"><section className="login-card oauth-card"><Wordmark /><div className="login-copy"><p>YOUR PRIVATE NOTEBOOK</p><h1>Write naturally.<br />Keep everything.</h1><span>Sign in to open your library and continue exactly where you left off.</span></div><div className="oauth-buttons"><button className="google-auth" onClick={() => onProvider("google")} disabled={!configured}><b>G</b> Continue with Google</button><button className="x-auth" onClick={() => onProvider("x")} disabled={!configured}>Continue with X</button><button className="discord-auth" onClick={() => onProvider("discord")} disabled={!configured}>Continue with Discord</button></div>{message && <p className="auth-error" role="alert">{message}</p>}<small>By continuing, you agree to use Penstyle for your personal notes. We never receive your social password.</small></section></main>;
}
...
function LibraryScreen({ profile, books, dark, setDark, onOpen, onDelete, onRename, onAppearance, onShare, onToggleImportant, newBookOpen, setNewBookOpen, onCreate, onImport, importRef, onSignOut }: { profile: Profile; books: Book[]; dark: boolean; setDark: (value: boolean) => void; onOpen: (book: Book) => void; onDelete: (book: Book) => void; onRename: (book: Book) => void; onAppearance: (book: Book) => void; onShare: (book: Book) => void; onToggleImportant: (book: Book) => void; newBookOpen: boolean; setNewBookOpen: (value: boolean) => void; onCreate: (event: FormEvent<HTMLFormElement>) => void; onImport: (event: React.ChangeEvent<HTMLInputElement>) => void; importRef: React.RefObject<HTMLInputElement | null>; onSignOut: () => void }) {
  ...
  return <main className={`library-screen ${dark ? "dark" : ""}`}>
    <header className="library-header"><Wordmark small /><div className="account"><div className="account-profile" title={profile.email}><span aria-hidden="true">{initials}</span>{profile.avatar && <img src={profile.avatar} width="28" height="28" alt="" aria-hidden="true" referrerPolicy="no-referrer" onError={(event) => { event.currentTarget.hidden = true; }} />}<strong>{profile.name}</strong></div><nav className="library-help" aria-label="Help"><a href="/documentation">Docs</a><a href="/guide">Guide</a></nav><button onClick={() => setDark(!dark)}><Settings2 size={16} /> {dark ? "Light" : "Dark"}</button><button className="sign-out" onClick={onSignOut}><LogOut size={16} /> Sign out</button></div></header>
